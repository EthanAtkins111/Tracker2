  ...
  return (
    <div className="space-y-4 animate-fade-in">
      <div className="flex items-center justify-between">
        ...
        <Button size="sm" onClick={() => { setEditId(null); setShowAdd(true); }}><Plus className="mr-1.5 h-3.5 w-3.5" /> Add Contact</Button>
      </div>
        ...
        {filtered.map(c => (
          <Card key={c.id} className="p-4">
            <div className="flex items-start justify-between">
              <div><p className="font-semibold text-sm">{c.name}</p><p className="text-xs text-muted-foreground">{c.role}</p></div>
              <div className="flex gap-1">
                <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => { setEditId(c.id); setShowAdd(true); }}><Edit className="h-3.5 w-3.5" /></Button>
                <Button size="icon" variant="ghost" className="h-7 w-7" onClick={async () => { await removeContact(c.id); refresh(); }}><Trash2 className="h-3.5 w-3.5" /></Button>
              </div>
            </div>
            ...
            <p className="text-xs text-primary mt-2 cursor-pointer hover:underline" onClick={() => navigate(`/accounts/${c.accountId}`)}>{getAccountName(c.accountId)}</p>
          </Card>
        ))}
      </div>
